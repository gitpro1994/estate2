...
if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
{
	...
	{		
		if (move_uploaded_file($_FILES["cover_photo"]["tmp_name"], $location)) 
		{
			...
			if ($execute) 
			{
				$title   = $title." başlıqlı yeni xəbər əlavə edildi";
	            $content = $title." başlıqlı yeni xəbər əlavə edildi. Əlavə edən IP adresi ".getIP()." , istifadə edilən Brauzer ".user_brauzer($_SERVER['HTTP_USER_AGENT'])."  ";
