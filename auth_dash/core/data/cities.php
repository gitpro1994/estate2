...
$sel                    = mysqli_query($conn,"SELECT COUNT(*) AS allcount FROM cities");
$records                = mysqli_fetch_assoc($sel);
$totalRecords           = $records['allcount'];

## Filterli umumi netice sayi
$sel                    = mysqli_query($conn,"SELECT COUNT(*) as allcount FROM cities WHERE 1 ".$searchQuery);
$records                = mysqli_fetch_assoc($sel);
$totalRecordwithFilter  = $records['allcount'];

## Cixan neticeler
$empQuery   = "SELECT * FROM cities WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
...
$sub_array[] = $bax['id'];
$sub_array[] = '<a href="'.base_url().'/edit_cities/'.$bax['id'].'" class="renk_baslik" title="'.translate('edit').'">'.$bax['city_name'].'</a>';
