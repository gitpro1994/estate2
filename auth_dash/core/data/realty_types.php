...
$searchQuery = " ";
if($searchValue != ''){
   $searchQuery = " AND (kind_name LIKE '%".$searchValue."%' ) ";
}

## Filtersiz umumi netice sayi
$sel                    = mysqli_query($conn,"SELECT COUNT(*) AS allcount FROM realty_types");
$records                = mysqli_fetch_assoc($sel);
$totalRecords           = $records['allcount'];

## Filterli umumi netice sayi
$sel                    = mysqli_query($conn,"SELECT COUNT(*) as allcount FROM realty_types WHERE 1 ".$searchQuery);
$records                = mysqli_fetch_assoc($sel);
$totalRecordwithFilter  = $records['allcount'];

## Cixan neticeler
$empQuery   = "SELECT * FROM realty_types WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
...
$sub_array[] = $bax['id'];
$sub_array[] = '<a href="'.base_url().'/edit_ads_type/'.$bax['id'].'" class="renk_baslik" title="'.translate('edit').'">'.$bax['type_name'].'</a>';
