   ...
   <div class="content-wrapper">
      ...
      <div class="row">
         <div class="col-md-12 grid-margin">
            <div class="card">
               <div class="card-body">
                  <form class="forms-sample" id="region_edit_form" method="post" action="" enctype="multipart/form-data">
                     <div class="form-group">
                        <label for="region_name"><?= translate('region_name') ?></label>
                        <input type="text" class="form-control form-control-sm" name="region_name" id="region_name" value="<?= regionById($_GET['id'],'region_name') ?>">
                     </div>                    
                     <div class="form-group">
                        ...
                        <select class="form-control form-control-sm" id="city" name="city">
                           ...
                           <?php 
                             $cid = regionById($_GET['id'],'city_id');
                             $cat = "SELECT * FROM cities WHERE status=1";
                             $cru = mysqli_query($conn,$cat);
                             while($n = mysqli_fetch_array($cru)){
                              ?>
                          <option value="<?= $n['id'] ?>" <?= ($n['id']==$cid) ? 'selected' : '' ?>><?= translate($n['city_name']); ?></option>
                           <?php } ?>
                        </select>
                      </div>
                      <input type="hidden" name="region_id" id="region_id" value="<?= $_GET['id'] ?>">
                     <div class="form-group mb-2">
                      <?php $status = (regionById($_GET['id'],'status')==1) ? 'checked' : ''; ?>						
                        <label class="switch">
                     ...
                     <button id="edit_region" type="submit" name="edit_region" class="btn btn-primary btn-icon-text btn-sm">
